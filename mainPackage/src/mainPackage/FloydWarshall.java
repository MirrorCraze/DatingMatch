...
public class FloydWarshall {
	public static ArrayList<Person> male = new ArrayList<>();
	public static ArrayList<Person> female = new ArrayList<>();
	...
	public static void mapInitialize() {
		...
		for(int i=0;i<station.size();i++)
		{
			for(int j=0;j<station.size();j++)
			{
				if(i==j)
					distStation[i][j] = 0;
				else distStation[i][j] = maxLength;
			}
		}
		...
		for(int i=0;i<station.size();i++)
		{
			for(int j=0;j<station.size();j++)
			{
				System.out.printf("%f ",distStation[i][j]);
			}
			System.out.printf("\n");
		}
	}
	...
	public static void addPerson(String name, double latCoor, double longCoor, char sex) {
		
		Person newPerson = new Person(name,latCoor,longCoor,sex);
		if(sex == 'M')
			male.add(newPerson);
		else female.add(newPerson);
	}
	public static double[][] calShortestDist()
	{
		double[][] dist = new double[male.size()][female.size()];
		...
		//find shortest path for male to female, using male->stationStart->stationEnd->female
		for(int i=0;i<male.size();i++)
		{
	...
	public static void printDist(double[][] dist)
	{
		...
		for(int i=0;i<female.size();i++)
		{
			System.out.printf("%15s",female.get(i).name);
		}
		System.out.println();
		for(int i=0;i<male.size();i++)
		{
			System.out.printf("%15s", male.get(i).name);
			for(int j=0;j<female.size();j++)
			{
	...
	public static double findTimeDiff(CoorPair pair1, CoorPair pair2)
	{
		...
		double dist = Math.sqrt(Math.pow((pair1.lat-pair2.lat), 2) + Math.pow((pair1.longi-pair2.longi), 2)); //normal distance formula. sqrt((diffX)^2 + (diffY)^2)
